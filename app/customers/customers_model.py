...
class Customers:
    def add_client(self, customer_name, customer_phone, customer_email, customer_address, customer_product, customer_contact_person):
        try:
            conn = dbInstance.connectToDatabase()
            cur = conn.cursor()
            cur.execute("INSERT INTO customers(customer_name,customer_phone,customer_email,customer_address,customer_product,customer_contact_person) VALUES(%s,%s,%s,%s,%s,%s)",
                        (customer_name, customer_phone, customer_email, customer_address, customer_product, customer_contact_person))
            conn.commit()
    ...
    def delete_a_client(self, client_id):
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "DELETE FROM customers WHERE customer_id=%s", [client_id])
            conn.commit()
